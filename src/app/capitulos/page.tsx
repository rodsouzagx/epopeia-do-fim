...
export default async function ChaptersPage() {
  // 1. Tenta buscar os volumes e capítulos do Sanity
  ...
  // 2. Se não houver volumes no Sanity, monta o fallback a partir do arquivo local
  let volumesToDisplay = sanityVolumes;
  if (!volumesToDisplay || volumesToDisplay.length === 0) {
    const map = ALL_CHAPTERS.reduce((acc: Record<number, any>, item) => {
      if (!acc[item.volume]) {
        acc[item.volume] = {
          _id: `vol-${item.volume}`,
    ...
    volumesToDisplay = Object.values(map);
  }
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-12">
        ...
        <div className="flex flex-col gap-10">
          {volumesToDisplay.map((vol: any, idx: number) => (
            ...
            >
              {/* Cabeçalho do Volume (com capa se existir) */}
